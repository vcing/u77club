app.controller('sideBarCtrl',['$scope','$stateParams','userSelf','roomListByIds','messageNew','roomInfo','roomUpdateActive',
	function($scope,$stateParams,userSelf,roomListByIds,messageNew,roomInfo,roomUpdateActive){
		var _name = 'sideBarCtrl';
		var _user = userSelf.self();
		// 当前激活房间ID
		$scope.current = $stateParams.roomId;
		$scope.user = _user;
		...
		if(!messageNew.checkListener(_name)){
			messageNew.addListener(_name,function(msg){
				angular.forEach($scope.rooms,function(room){
					...
					if(room._id == msg.room && $scope.current != room._id){
